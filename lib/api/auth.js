    ...
    server.post(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                oauth: booleanSchema.default(false),

                password: Joi.string().max(256).required()
                    .when('oauth', {is: true, then: Joi.any()}),
            ...
            if (result.value.oauth) {
                try {
                    [authData, user] = await userHandler.asyncOauth(result.value.username, result.value.scope, meta);
                } catch (err) {
                    let response = {
                        error: err.message,
                        code: 'OAuthFailed' || err.code
                    };
                    if (user) {
                ...
                if (!authData) {
                    let response = {
                        error: 'OpenAuthentication failed',
                        code: 'OAuthFailed'
                    };
                    if (user) {
                        response.id = user.toString();
