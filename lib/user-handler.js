...
class UserHandler {
     ...
     * @param {String} [meta.session] Session ID
     */
    oauth(username, requiredScope, meta, callback) {
        if (!callback) {
            return this.asyncOauth(username, requiredScope, meta);
        }
        this.asyncOauth(username, requiredScope, meta)
            .then(result => {
                if (!Array.isArray(result)) {
                    result = [].concat(result || [false, false]);
                }
    ...
    async asyncOauth(username, password, requiredScope, meta) {
        meta = meta || {};
        requiredScope = requiredScope || 'master';
        ...
        let authResponse = {
            ...
            // if 2FA is enabled then require token validation
            require2fa: false,
            requirePasswordChange: false // true, if password was reset and using temporary password
        };
